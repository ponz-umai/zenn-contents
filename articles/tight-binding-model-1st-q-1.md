...
（この後の章^[[飛び移り積分（Hopping Integral）の物理的意味・Wannier関数の従う方程式](https://zenn.dev/ponzumai/articles/tight-binding-model-hopping-int)、 [重なり積分（Overlap Integral）の物理的意味（の妄想）](https://zenn.dev/ponzumai/articles/tight-binding-model-overlap-int)]で、さらに怪しい議論を展開して重なり積分が小さいことを納得していく予定です）
